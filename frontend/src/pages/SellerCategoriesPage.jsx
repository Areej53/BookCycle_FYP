...
export default function SellerCategoriesPage() {
    ...
    const handleCategoryClick = (cat) => {
        if (isRentMode) {
            navigate(`/explore?cats=${encodeURIComponent(cat)}&type=rent`);
            return;
        }
    ...
    const isSelected = (cat) => !isRentMode && sellerData.category === cat;
            ...
            <div className="page-layout">
                <main>
                    <div className="cat-header">
                        {isRentMode ? (
                            <>
                                <div className="cat-tag"><span className="cat-tag-dot"></span>BookCycle Rental Shelf</div>
                                <h1 className="cat-title">Browse Rent <em>Categories</em></h1>
                                <p className="cat-sub">Choose a category to browse rental books. Find your favorite books, rent them at fixed rates, and return when done!</p>
                            </>
                        ) : (
                            <>
                    ...
                    <div className="cat-grid">
                        {[{id: 'programming', name: 'Programming', desc: 'CS, coding, algorithms, software engineering & tech.', count: 148, img: 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?w=150&q=80'},
                            ...
                            <div key={cat.id} className={`cat-card ${isSelected(cat.name) ? 'selected' : ''}`} onClick={() => handleCategoryClick(cat.name)}>
                                {!isRentMode && <div className={`cat-check ${isSelected(cat.name) ? 'active' : ''}`}></div>}
