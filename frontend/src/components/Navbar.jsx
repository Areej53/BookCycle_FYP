    ...
    <>
      <nav style={{
        ...
        <ul className="nav-links" style={{ display: 'flex', alignItems: 'center', gap: '25px', margin: 0, padding: 0 }}>
          ...
          {location.pathname !== '/' && (
            <li>
              <Link to="/dashboard" style={{ display: 'flex', alignItems: 'center', position: 'relative', color: location.pathname.startsWith('/dashboard') ? 'var(--accent)' : 'rgba(255,250,224,.82)', transition: 'color .2s' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--accent)'} onMouseLeave={e => e.currentTarget.style.color = location.pathname.startsWith('/dashboard') ? 'var(--accent)' : 'rgba(255,250,224,.82)'}>
                <MdDashboard size={22} />
                {unreadCount > 0 && (
                  <span style={{
                    position: 'absolute', top: '-6px', right: '-12px',
